...
pub fn get_enhanced_displays(needs_caps: bool) -> Result<Vec<Display>, Error> {
...
pub fn get_enhanced_display(needs_caps: bool, id: usize) -> Result<Display, Error> {
    get_enhanced_displays(needs_caps)?
        .into_iter()
        .nth(id)
...
pub fn get_brightness(id: usize) -> Result<VcpValue, Error> {
    let mut display = get_enhanced_display(true, id)?;
